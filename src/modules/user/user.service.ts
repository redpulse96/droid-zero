...
export class UserService extends BaseService<Users> {
  ...
  public async login(email: string, password: string, token: string) {
    ...
    if (user) {
      this.logger.info(`Attempting to login user: ${user.email}`);
      // Check if the account is locked
      if (user.locked) {
        this.logger.warn('Users tried to log into locked account');
  ...
  public async createUser(email: string) {
    ...
    this.logger.info(`Users entered into DB: ${newUser.email}`);
