...
import {
  CreatePricingsDto,
  FetchPricingDetailsDto,
} from './dto/pricings-input.dto';
  ...
  public async fetchProductListByFilter(
    ...
    try {
      const filter: any = {
        ...
        name: pricings_filter?.name
          ? Like(`%${pricings_filter.name}%`)
          : undefined,
        description: pricings_filter?.description
          ? Like(`%${pricings_filter.description}%`)
          : undefined,
        code: pricings_filter?.code
          ? Like(`%${pricings_filter.code}%`)
          : undefined,
        status: Status.Active,
      };
