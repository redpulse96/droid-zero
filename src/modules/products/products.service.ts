...
import { CreateProductsDto, FetchProductDetailsDto } from './dto/products-input.dto';
...
export class ProductService extends BaseService<Products> {
  ...
  public async createProducts(product_items: CreateProductsDto[]): Promise<InterfaceList.MethodResponse> {
    try {
      ...
      const [createError, products]: any[] = await executePromise(this.create(createProductsArr));
      if (createError) {
  ...
  public async fetchProductListByFilter(products_filter: FetchProductDetailsDto): Promise<InterfaceList.MethodResponse> {
    try {
      const filter: any = {
        id: products_filter?.id ? products_filter.id : undefined,
        name: products_filter?.name ? products_filter.name : undefined,
        category: products_filter?.category_id ? products_filter.category_id : undefined,
        subcategory: products_filter?.subcategory_id ? products_filter.subcategory_id : undefined,
      ...
      const [productsError, products]: any[] = await executePromise(this.findAll(filter));
      if (productsError) {
